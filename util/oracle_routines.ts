  ...
  try {
    ...
    for (const declaration of req.body.declarations) {
      const newDeclaration = await connection.execute(
        "INSERT INTO vTrack.NWD_VESSEL_TRANSACTIONS ( VESSEL_PASSCODE, VESSEL_DOC_NUMBER, TRANSACTION_TYPE, CONFIRMATION_NUMBER, TRANSACTION_DATE, TRANSACTION_CONTACT_NAME, VMS_TECH, COMMENTS, TRANSACTION_CODE) VALUES (:VESSEL_PASSCODE, :VESSEL_DOC_NUMBER, :TRANSACTION_TYPE, :CONFIRMATION_NUMBER, TO_DATE(:TRANSACTION_DATE, 'DD-MMMM-YY'), :TRANSACTION_CONTACT_NAME, :VMS_TECH, :COMMENTS, :TRANSACTION_CODE)", [declaration.VESSEL_PASSCODE, declaration.VESSEL_DOC_NUMBER, declaration.TRANSACTION_TYPE, newConfNum.toString(), moment(declaration.TRANSACTION_DATE).format('DD-MMMM-YY'), declaration.TRANSACTION_CONTACT_NAME, declaration.VMS_TECH, declaration.COMMENTS, declaration.TRANSACTION_CODE]
      )
